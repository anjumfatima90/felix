...
public class ComponentMetaDataCacheTest extends BaseTest {
	...
	private void doTestCachedComponentMetadata(String xmlFile, DSVersion version) throws IOException, XmlPullParserException, Exception {
		final List<?> metadataList = readMetadata(xmlFile);
	    assertEquals("Component Descriptors size not as expected", 1, metadataList.size());
	...
	private static void assertEqualsCM(ComponentMetadata actualCM, ComponentMetadata cachedCM) {
		assertEquals("DS version is not equal", actualCM.getDSVersion(), cachedCM.getDSVersion());
		assertEquals("Component name is not equal", actualCM.getName(), cachedCM.getName());
		assertEquals("Activate method name not equal", actualCM.getActivate(), cachedCM.getActivate());
		assertEquals("Activate method declaration not equal", actualCM.isActivateDeclared(), cachedCM.isActivateDeclared());
		assertEquals("Activate fields not equal", actualCM.getActivationFields(), cachedCM.getActivationFields());
		assertEquals("Deactivate method name not equal", actualCM.getDeactivate(), cachedCM.getDeactivate());
		assertEquals("Deactivate method declaration not equal", actualCM.isDeactivateDeclared(), cachedCM.isDeactivateDeclared());
		assertEquals("Modified method name not equal", actualCM.getModified(), cachedCM.getModified());
		assertEquals("Configuration Policy not equal", actualCM.getConfigurationPolicy(), cachedCM.getConfigurationPolicy());
		assertEquals("Configuration Pid not equal", actualCM.getConfigurationPid(), cachedCM.getConfigurationPid());
		assertEquals("Factory Identifier not equal", actualCM.getFactoryIdentifier(), cachedCM.getFactoryIdentifier());
		assertEquals("Factory check not equal", actualCM.isFactory(), cachedCM.isFactory());
		assertEquals("Activation policy flag not equal", actualCM.isImmediate(), cachedCM.isImmediate());
		assertEquals("Implementation Class Name not equal", actualCM.getImplementationClassName(), cachedCM.getImplementationClassName());
		assertEquals("Number of Init parameters not equal", actualCM.getNumberOfConstructorParameters(), cachedCM.getNumberOfConstructorParameters());
	...
	private static void checkServiceMetadata(ComponentMetadata actualCM, ComponentMetadata cachedCM) {
		...
		if(sm1 != null && sm2 != null) {
			...
			assertEquals("Service Provides length not equal", implementedInterfaces1.length, implementedInterfaces2.length);
			for(int i = 0; i < implementedInterfaces1.length; i++) {
				assertEquals("Implemented interfaces not same", implementedInterfaces1[i], implementedInterfaces2[i]);
			}
		}
	}

	private static void checkReferenceMetadata(ComponentMetadata actualCM, ComponentMetadata cachedCM) {
		List<ReferenceMetadata> references1 = actualCM.getDependencies();
		List<ReferenceMetadata> references2 = cachedCM.getDependencies();
		assertEquals("Number of references not same", references1.size(), references2.size());
		for(int i = 0; i < references1.size(); i++) {
			...
			assertEquals("Reference interface not equal", r1.getInterface(), r2.getInterface());
			assertEquals("Reference cardinality not equal", r1.getCardinality(), r2.getCardinality());
			assertEquals("Reference policy not equal", r1.getPolicy(), r2.getPolicy());
			assertEquals("Reference policy option not equal", r1.getPolicyOption(), r2.getPolicyOption());
			assertEquals("Reference target not equal", r1.getTarget(), r2.getTarget());
			assertEquals("Reference target property name not equal", r1.getTargetPropertyName(), r2.getTargetPropertyName());
			...
			assertEquals("Reference field option not equal", r1.getFieldOption(), r2.getFieldOption());
			assertEquals("Reference field collection type not equal", r1.getFieldCollectionType(), r2.getFieldCollectionType());
			assertEquals("Reference scope not equal", r1.getScope(), r2.getScope());
			assertEquals("Reference parameter index not equal", r1.getParameterIndex(), r2.getParameterIndex());		
			assertEquals("Reference parameter collection type not equal", r1.getParameterCollectionType(), r2.getParameterCollectionType());
		}
	}

	

	private static void checkProperties(Map<String, Object> properties1, Map<String, Object> properties2) {
	...
	private static void checkPropertyValues(Object value1, Object value2) {
		assertEquals("Property Values type not same", value1.getClass(), value2.getClass());
