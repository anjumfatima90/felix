...
public class ComponentMetaDataCacheIntegrationTest extends ComponentTestBase {
	...
	public void testCacheExists() throws Exception {
		Collection<ComponentDescriptionDTO> actualComponents = getComponentDescriptions(bundle);
		...
		Collection<ComponentDescriptionDTO> cachedComponents = getComponentDescriptions(bundle);
	...
	public void testUpdateBundleDismissesCache() throws Exception {
		...
		Collection<ComponentDescriptionDTO> bundleAComponents = getComponentDescriptions(bundle);
		...
		bundle.update(createBundleInputStream(descFile, COMPONENT_PACKAGE, "simplecomponent", "0.0.12"));

		Collection<ComponentDescriptionDTO> bundleBComponents = getComponentDescriptions(bundle);
		...
		Collection<ComponentDescriptionDTO> cachedBundleBComponents = getComponentDescriptions(bundle);
		...
		bundle.update(createBundleInputStream(descriptorFile, COMPONENT_PACKAGE, "simplecomponent", "0.0.11"));

		scrBundle.start();

		Collection<ComponentDescriptionDTO> bundleCComponents = getComponentDescriptions(bundle);
	...
	private void assertComponentsEqual(List<ComponentDescriptionDTO> actualComponents,
			List<ComponentDescriptionDTO> cachedComponents) throws InvocationTargetException, InterruptedException {
		assertEquals("Number of components not equal", actualComponents.size(), cachedComponents.size());
		for (int i = 0; i < actualComponents.size(); i++) {
			...
			assertEquals("Component Name not equal", actualComponent.name, cachedComponent.name);
			assertEquals("Expecting components to be enabled", actualComponent.defaultEnabled,
					cachedComponent.defaultEnabled);

			ComponentConfigurationDTO actualCC = findComponentConfigurationByName(actualComponent.name, 0);
			ComponentConfigurationDTO cachedCC = findComponentConfigurationByName(cachedComponent.name, 0);
			assertEquals("Expecting component states to be equal", actualCC.state, cachedCC.state);

			checkSatisfiedReferences(actualCC.satisfiedReferences, cachedCC.satisfiedReferences);

			checkProperties(actualCC.properties, cachedCC.properties);

			if (actualCC.service != null && cachedCC.service != null) {
				assertEquals("Service Reference not same", actualCC.service.toString(), cachedCC.service.toString());
			}
		}
	}

	private static void checkSatisfiedReferences(SatisfiedReferenceDTO[] satisfiedReferences1,
			SatisfiedReferenceDTO[] satisfiedReferences2) {
		assertEquals("Size of satisfied references not equal", satisfiedReferences1.length,
				satisfiedReferences2.length);
		for (int i = 0; i < satisfiedReferences1.length; i++) {
			assertEquals("Satisfied referneces name not equal", satisfiedReferences1[i].toString(),
					satisfiedReferences2[i].toString());
		}
	}

	private static void checkProperties(Map<String, Object> actualProperties, Map<String, Object> cachedProperties) {
		assertEquals("Properties size not equal", actualProperties.size(), cachedProperties.size());
		for (String property : actualProperties.keySet()) {
			assertTrue("Properties not equal", cachedProperties.containsKey(property));
			checkPropertyValues(actualProperties.get(property), cachedProperties.get(property));
		}
	}
